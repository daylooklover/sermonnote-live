...
import { getFunctions, httpsCallable, HttpsCallableResult, connectFunctionsEmulator } from 'firebase/functions'; // <-- connectFunctionsEmulator 임포트 추가

// Firebase 설정
// 로컬 개발 환경에서는 __firebase_config 전역 변수가 정의되지 않습니다.
// 따라서 여기에 실제 Firebase 프로젝트 설정 값을 직접 입력해야 합니다.
// 이 설정은 Firebase 콘솔에서 "프로젝트 설정" -> "내 앱" 섹션에서 찾을 수 있습니다.
const firebaseConfig = {
  apiKey: "AIzaSyDuF64w0S6ZcoXAuhtahlUAhCgAYnOFBXo", // <-- 여기에 실제 API Key 입력
  authDomain: "sermonnote-live.firebaseapp.com", // <-- 여기에 실제 Auth Domain 입력
  projectId: "sermonnote-live", // <-- 여기에 실제 Project ID 입력
  storageBucket: "sermonnote-live.firebasestorage.app", // <-- 여기에 실제 Storage Bucket 입력
  messagingSenderId: "520754190508", // <-- 여기에 실제 Messaging Sender ID 입력
  appId: "1:520754190508:web:cc4271b1e96e0bed3ee709", // <-- 여기에 실제 App ID 입력
};
...
try {
  if (firebaseConfig.apiKey && firebaseConfig.projectId) {
    ...
    // --- 이 부분이 중요합니다 ---
    // 로컬 Functions 에뮬레이터에 연결
    // Firebase Functions 에뮬레이터가 실행 중인 포트와 IP를 확인하여 정확히 입력해야 합니다.
    // 에뮬레이터 시작 시 터미널에 출력되는 URL을 확인하세요. (예: http://127.0.0.1:5001)
    // 만약 5000에서 시작했다면 5000으로, 5001에서 시작했다면 5001로 맞춰주세요.
    // 대부분의 경우 "127.0.0.1" 또는 "localhost"를 사용합니다.
    connectFunctionsEmulator(functionsInstance, "127.0.0.1", 5001); // <-- 이 포트 번호를 에뮬레이터 로그와 일치시키세요.
    // --- 추가 끝 ---

  } else {
    console.error("오류: Firebase 설정이 유효하지 않습니다. firebaseConfig 객체의 필수 필드가 비어있습니다.");
  }
} catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4 font-inter">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="query" className="block text-gray-700 text-sm font-semibold mb-2">
              질문을 입력하세요:
            </label>
            <textarea
              id="query"
